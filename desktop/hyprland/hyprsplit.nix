...
{
  options.liyua.desktop.hyprland.hyprsplit.enable =
    lib.mkEnableOption "Separate workspaces for each monitor";
        ...
        let
          genWorkspaceKeybinds =
            ...
            (
              ...
              |> map (
                x:
                "${mod}, ${x |> (x: x - builtins.floor (x / end) * end) |> builtins.toString}, ${
                  if enable then "split:${rule}" else rule
                }, ${builtins.toString x}"
              )
            );
        in
