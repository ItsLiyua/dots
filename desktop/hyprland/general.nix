{ config, lib, ... }:
{
  options.liyua.desktop.hyprland.general.enable =
    lib.mkEnableOption "Hyprland general configuration options";
  config = {
    liyua.desktop.hyprland.general.enable = config.liyua.desktop.hyprland.enable;
    wayland.windowManager.hyprland.settings.general =
      lib.mkIf config.liyua.desktop.hyprland.general.enable
        {
          gaps_in = 5;
          gaps_out = 10;
          border_size = 2;
