{
  ...
  config = let
    monitorProps = builtins.map (m: lib.splitString "," m) config.wayland.windowManager.hyprland.settings.monitor;
  ...
  in
    lib.mkIf config.liyua.desktop.wallpaper.enable {
      ...
      programs.hyprlock.settings.background = lib.mkForce (lib.map (w: {
          monitor = w.name;
          path = w.file;
          blur_passes = 2;
        })
