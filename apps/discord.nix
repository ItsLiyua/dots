{ config, lib, ... }:
{
  options.liyua.apps.discord.enable = lib.mkEnableOption "Discord (nixcord)";
  config = lib.mkIf config.liyua.apps.discord.enable {
    ...
    home.file.".config/Vencord/themes/system24.theme.css".text =
      with config.lib.stylix.colors.withHashtag; ''
        ...
        */
        ...
        :root {
            ...
            --text-1: oklch(from var(--text-3) min(calc(l + 0.2), 1) c h); /* other normally white text */
            --text-2: oklch(from var(--text-3) min(calc(l + 0.2), 1) c h); /* headings and important text */
            ...
            --hover: oklch(from var(--text-3) l c h / 0.1); /* channels and buttons when hovered */
            --active: oklch(from var(--text-3) l c h / 0.2); /* channels and buttons when clicked or selected */
            --active-2: oklch(from var(--text-3) l c h / 0.3); /* extra state for transparent buttons */
